...
An executor may optionally expose a **trusted read-only Ruby bundle seed** through `RUNDIFF_SUBJECT_BUNDLE_SEED_ROOT`. The seed is keyed by Ruby major/minor plus the exact `Gemfile.lock` digest. RunDiff never executes directly from that seed and never shares one writable bundle directory between baseline and candidate. Instead, only the seed's bundle payload is copied into each worktree-local cache before ownership is handed to the subject identity. Bundler configuration is not copied. A missing key falls back to the normal isolated install path. Production Lab uses this only for its repository-owned synthetic Rails fixture so repeated A/B proofs do not repeatedly download the same trusted dependency graph.
